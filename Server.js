...
function extractAccessToken(err, remoteResponse, remoteBody,res){
	...
	console.log('sfdcResponse: '+sfdcResponse) ;
	//success
...
app.all('/proxy',  function(req, res) {     
    ...
    request({ url: url, method: req.method, json: req.body, 
                    headers: {'Authorization': req.header('X-Authorization'), 'Content-Type' : 'application/json', 'Content-Length' : 0}, body:req.body }).pipe(res); 
});
...
app.get('/webServerStep2', function (req,res){  
	...
	var paramBody = 'client_id='+ jwt_consumer_key + 
